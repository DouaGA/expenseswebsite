...
def claim_stats(request):
    ...
    current_date = start_date
    while current_date <= end_date:
        date_counts[current_date] = 0  # Stocke des objets date
        current_date += timedelta(days=1)
    ...
    sorted_dates = sorted(date_counts.items(), key=lambda x: x[0])  # Trie par date
    dates = [date.strftime('%Y-%m-%d') for date, count in sorted_dates]  # Convertit en strings pour JavaScript
    ...
    try:
        ...
        data = {
            ...
            'location': {
                'lat': claim.location_lat,
                'lng': claim.location_lng,
                'address': claim.municipality.name if claim.municipality else None
            },
            'created_at': claim.created_at.isoformat(),
            'updated_at': claim.updated_at.isoformat() if claim.updated_at else None,
            'admin_comment': claim.admin_comment,
            'created_by': {
                'id': claim.created_by.id,
                'username': claim.created_by.username
            } if claim.created_by else None,
            'edit_url': f"/admin/claims/claim/{claim.id}/change/"
        }
